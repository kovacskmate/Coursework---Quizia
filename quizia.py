...
def play(quiz_id):
    db = get_db() 
    cur = db.cursor()  
    sql = "SELECT COUNT(*) FROM quizzes WHERE quiz_id = ?"
    result = cur.execute(sql, (quiz_id, ))
...
def paging(page, sql, template):
    ...
    result = cur.execute(sql, (startLimit,21))    
    rowcount = result.fetchall()
    disableNext = 1
    data = []
    if len(rowcount) == 0 and int(page) == 1:
        ...
        for item in cur.execute(sql, (startLimit, 20)):
            data.append(item)
        if len(rowcount) > 20:
            disableNext = 0
        return render_template(template, name=current_user.username, data=data, page=page, disableNext=disableNext)
...
def categoriesCategory(category, page):
    ...
    result = cur.execute(sql, (category, ))
    rowcount = result.fetchall()
    if rowcount[0][0] > 0:
...
def _saveQuizToDB():
    ...
    try:
        ...
        cursor.execute(sql, (f['selectCategory'], current_user.id, f['quizName']))        
...
def _updateChallengedb():    
    ...
    for i in range(int(whatToUpdate['numberOfQuestions'])):
        sql = "UPDATE questions SET attempts = attempts + 1, correct_attempts = correct_attempts + ? WHERE question_id = ?"
        db.cursor().execute(sql, (whatToUpdate['questions[' + str(i) + '][correct_attempts]'], whatToUpdate['questions[' + str(i) + '][question_id]']))    
    sql = "UPDATE user SET challenge_score = challenge_score + ? WHERE username = ?"
...
def profile(id):
    ...
    if rowcount[0][0] > 0:
        ...
        userdata = db.cursor().execute(sql, (userid, userid, userid)).fetchall()       
        ...
        userachievements = db.cursor().execute(sql, (userid, )).fetchall()
...
def editProfile():
    ...
    userdata = db.cursor().execute(sql, (current_user.id, )).fetchall()
    ...
    try:                 
        ...
        if not f['introduction'] and file.filename:
            ...
            if allowed_file(filename):
                ...
                cursor.execute(sql, (filePath, current_user.id))
                db.commit()
            else:
                raise Exception("Wrong file format")
            ...
            cursor.execute(sql, (introduction, current_user.id))    
            db.commit()
        else:
            introduction = f['introduction']
            filename = secure_filename(file.filename)
            if allowed_file(filename):
                ...
                cursor.execute(sql, (filePath, introduction, current_user.id)) 
                db.commit()
            else:
                raise Exception("Wrong file format")
